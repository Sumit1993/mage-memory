...
The execution of [ADR-0007 (now ADR-0050)](../decisions/0050-mage-turns-a-repeated-failure-into-enforcement.md). agentmemory
...
- **Adopted, re-shaped:** per-harness adapter-installer → `mage connect` (ADR-0009 (now ADR-0054)) ·
  auto-capture → deterministic stage + agent distill (ADR-0009) · privacy filter → the redaction
  gates · dashboard → no-server generated `dashboard.html` (ADR-0020 (now ADR-0054)) · their eval harness
...
- **Rejected:** server-sharing, multi-agent leases/signals (ADR-0010 (now ADR-0050)), any daemon/vector-DB in
  core (ADR-0007/0009); vector recall, if ever, is an opt-in MCP accelerator.

## Resist (porting these would betray the thesis)

Silent capture-everything (contradicts ADR-0004 (now ADR-0051) + the human-confirm gate) · a server of record ·
coordination primitives (ADR-0010 — orchestration, not memory).

## Relations

- implements [ADR-0007 — mine agentmemory's design, don't depend](../decisions/0050-mage-turns-a-repeated-failure-into-enforcement.md)
- detailed_by [ADR-0009](../decisions/0054-files-in-the-repo-no-runtime-no-server.md) ·
  [ADR-0010](../decisions/0050-mage-turns-a-repeated-failure-into-enforcement.md)
