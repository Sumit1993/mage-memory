...
2. **npm package = `mage-memory`** (exact `mage` is taken on npm; `mage-memory` is exact-free). **CLI command = `mage`**, via `bin`:
   ...
   - Data dir = `mage/` (visible — **revised by [ADR-0008 (now ADR-0054)](../decisions/0054-files-in-the-repo-no-runtime-no-server.md)**; was `.mage/` in v0.1, un-dotted so the in-repo base opens as an Obsidian vault); metadata schema = `mage.v1`; skills namespaced `/mage:learn`, `/mage:dream`, etc.
...
Build handoff = [PLAN-v0.1](../work/plan-v0.1.md) + this file + [decisions 0001–0006](../decisions/0050-mage-turns-a-repeated-failure-into-enforcement.md) + [context & glossary](context.md).

## Relations
- governs [mage v0.1 implementation plan](../work/plan-v0.1.md)
- see_also [mage roadmap](../work/roadmap.md)
- revised_by [ADR-0008 — visible mage/ dir for Obsidian](../decisions/0054-files-in-the-repo-no-runtime-no-server.md)
