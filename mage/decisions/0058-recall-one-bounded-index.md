...
keywords: [recall, index, roster, MEMORY.md, INDEX.md, footprint, context-match, skills, trigger]
---
...
roster lists only notes that passed the ladder, ranked by admission order, and is bounded by the harness cap
(25,600 bytes, 200 lines); `footprint` warns at 70 percent, `doctor` fails at 90, and nothing is silently
truncated. Per-wing skills are generated and auto-load; a skill firing is counted by context-match over a
...
## Example
`mage footprint`: "MEMORY.md 19,201 of 25,600 bytes (75%): warn. INDEX.md 37 notes. Wing skill mage: 4 loads,
...
`src/metrics/context-match.ts:16`. Roster to admitted-only: #245. ADR keywords in the governance lines: #135.
