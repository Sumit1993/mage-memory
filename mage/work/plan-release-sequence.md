...
model-free. That is exactly the [autonomy track (ADR-0029 (now ADR-0052)–0036)](#the-autonomy-track--what-010-now-delivers-adr-00290036)
...
capabilities (ADR-0015 (now ADR-0052)/0016 pre-resolved the cross-cutting parts, so several artificial
...
| **0.0.3** | Skills ship as a Claude Code **plugin** (`mage:` namespace, bare names); `mage init` prints `/plugin install`; ship `skills` + `.claude-plugin` in `files[]`. **Pulled forward: Redaction Gate 2 (`mage redact`, ADR-0014 (now ADR-0053)) + the `mage:learn --from` skill prose** | 0013, 0014 | 0.0.2 | locked | **shipped** |
...
| **0.0.10** | **coherence**: vocabulary ADR (*every flavor is a knowledge base*; de-overload "hub") + **hub flat-vs-nested grill** → ADR + the `mage link` scaffold-consistency fix + **SDD skills removed** (the deferred ADR-0001 (now ADR-0050)/0002 prune) | **0022, 0023** | 0.0.9 | grilled⁷ | **shipped** |
...
See [the release-bump gotcha](../notes/release-bump-touches-many-artifacts.md) + [the typecheck-gap gotcha](../notes/test-files-were-excluded-from-typecheck.md).
...
(becomes **ADR-0024 (now ADR-0057)**), **building**. Ships the LESSON path (a1 = observed organic note creation, CC-memory
...
`.staging/`, surfaces via `additionalContext`, wired by `mage connect` — finishing [ADR-0009 (now ADR-0054)](../decisions/0054-files-in-the-repo-no-runtime-no-server.md)
...
dogfooded + adversarially reviewed. The **`.mage/` state fold ([ADR-0025 (now ADR-0054)](../decisions/0054-files-in-the-repo-no-runtime-no-server.md))
...
¹⁰ **Docs site** — **GRILLED 2026-06-18**, decisions locked in [plan-docs-site](plan-docs-site.md) → **[ADR-0026 (now ADR-0059)](../decisions/0059-versions-are-mechanical-the-release-is-two-counts.md)** (accepted).
...
scope beyond 0.1.0 — it *is* the cut. The [live multi-KB soak](../notes/soak-targets.md) drove every step; each
ADR fixes a failure the previous rung's gate could not see.

| ADR | The move | Status | Landed |
|---|---|---|---|
| **[0029](../decisions/0052-streams-and-the-observe-schema.md)** digest→agent | Deterministic core **narrows + compresses** into a digest; the **host model judges** (two replay gates KILLED model-free *selection* — no classifier isolates "durable lesson"). Engine stays model-free (ADR-0009). | active · **built** | 0.0.12 |
| **[0030](../decisions/0057-a-guard-lands-by-pull-request.md)** autonomy dial | Opt-in **Operator → Approver → Overseer** (HITL→HOTL). *Anything a human does at `mage:groom`, a capable agent can do* — only the **commit** is irreducible. The two-channel nudge is the low rung; the same tally is the work-list higher up. | active · **built** | 0.0.12 |
| **[0031](../decisions/0057-a-guard-lands-by-pull-request.md)** provenance | Writer stamps `provenance`/`autonomy` at creation → the **keep-vs-`git revert` ledger** (the one signal that higher autonomy is worth it) is measurable. | active · **Phase 1 built**; Phase 2 (reject-ledger) sketched | 0.0.12 |
| **[0032](../decisions/0051-the-ladder-runs-before-anything-is-remembered.md)** capture-redirect | Inline `mage stage` was *never run* — the native-memory write reflex won. **Co-opt it**: relocate the host memory dir onto the KB (`autoMemoryDirectory`), `PreToolUse` redirect, `PostToolUse` mirror. Capture always **ends in git**. | accepted · **built** | PR #45 |
| **[0033](../decisions/0058-recall-one-bounded-index.md)** recall-import | Agent ignored "read `INDEX.md` first." Emit a CC-auto-loaded **`MEMORY.md`** twin of the bounded index (fallback `@import mage/INDEX.md`): a volitional pull → a deterministic launch-load that survives `/compact`. Spike PASSED 06-27. | accepted · **built** | on `main` |
| **[0034](../decisions/0055-connect-doctor-migrate.md)** adopt | `mage adopt` onboards what predates the hook — a dispatcher: in-shape → **place**, out-of-shape → `learn --from` **distills** (never a copy). | accepted · **built, dogfooded** | PR #46 |
| **[0035](../decisions/0051-the-ladder-runs-before-anything-is-remembered.md)** notes-are-memories | The load-bearing insight: **a mage note IS an agent memory** — one store. Stop fighting frontmatter at write-time; scrub secrets before disk, **normalize at the durable boundary** (PostToolUse-flatten + commit backstop); `groom` = periodic curation. | accepted · **impl building** | on `main` |
| **[0036](../decisions/0054-files-in-the-repo-no-runtime-no-server.md)** defer-adapter | One harness is a *hypothetical* seam — **don't** build `HarnessAdapter` yet; consolidated CC note-shape into one `cc-note` module now (retired `schema-map.ts`). | accepted · **built** | on `main` |
...
2. **Finish the tail** — ADR-0035 (now ADR-0051) normalization hardening (PostToolUse-flatten vs restamp timing + the commit-time backstop) and **ADR-0031 (now ADR-0057) Phase 2** (the reject-ledger reconciler).
...
[ADR-0013 (now ADR-0057)](../decisions/0057-a-guard-lands-by-pull-request.md), must also carry
...
[ADR-0015](../decisions/0052-streams-and-the-observe-schema.md), so everything downstream
...
the coordination layer [ADR-0010](../decisions/0050-mage-turns-a-repeated-failure-into-enforcement.md)
...
files*, the human *commits the diff*. ADR-0006 's (now ADR-0058) "promotion deferred until wings
...
(ADR-0030 (now ADR-0057)). So 0.1.0 = the full self-grooming loop **that the agent drains for you**, up to the autonomy
...
The 2026-06-06 observe grill ([ADR-0015](../decisions/0052-streams-and-the-observe-schema.md)
+ [ADR-0016 (now ADR-0057)](../decisions/0057-a-guard-lands-by-pull-request.md)) pre-resolved
...
- **MCP recall accelerator** — an opt-in, out-of-core MCP server exposing search/get over
  ...
  coordination layer [ADR-0010](../decisions/0050-mage-turns-a-repeated-failure-into-enforcement.md)
...
- **Literal SkillOpt bridge** — export skills + trajectories to Microsoft's SkillOpt
  ...
  ships SkillOpt's *rails* in 0.0.8, not its harness. [ADR-0013](../decisions/0057-a-guard-lands-by-pull-request.md)
...
- detailed_by [ADR-0013 — procedure skills + the self-grooming loop](../decisions/0057-a-guard-lands-by-pull-request.md)
- detailed_by [ADR-0014 — two-gate redaction](../decisions/0053-redaction-two-gates-one-engine.md)
- detailed_by [ADR-0015 — mage observe capture schema](../decisions/0052-streams-and-the-observe-schema.md)
- detailed_by [ADR-0016 — context-match, the confidence ladder, and the single applier](../decisions/0057-a-guard-lands-by-pull-request.md)
- detailed_by [ADR-0017 — mage connect: the host hook adapter](../decisions/0055-connect-doctor-migrate.md)
- detailed_by [ADR-0018 — mage distill: the observed-scratch reader](../decisions/0052-streams-and-the-observe-schema.md)
- detailed_by [ADR-0019 — mage promote: self-grooming](../decisions/0057-a-guard-lands-by-pull-request.md)
- autonomy_pivot [ADR-0029 — digest-to-agent capture](../decisions/0052-streams-and-the-observe-schema.md)
- autonomy_dial [ADR-0030 — opt-in agent autonomy ladder](../decisions/0057-a-guard-lands-by-pull-request.md)
- autonomy_evidence [ADR-0031 — programmatic provenance + reject-ledger](../decisions/0057-a-guard-lands-by-pull-request.md)
- autonomy_capture [ADR-0032 (now ADR-0051) — capture-redirect native memory](../decisions/0051-the-ladder-runs-before-anything-is-remembered.md)
- autonomy_recall [ADR-0033 (now ADR-0058) — recall: import the bounded index](../decisions/0058-recall-one-bounded-index.md)
- autonomy_backfill [ADR-0034 (now ADR-0055) — adopt pre-existing knowledge](../decisions/0055-connect-doctor-migrate.md)
- autonomy_unify [ADR-0035 — notes are memories: one unified store](../decisions/0051-the-ladder-runs-before-anything-is-remembered.md)
- autonomy_seam [ADR-0036 (now ADR-0054) — defer the HarnessAdapter seam](../decisions/0054-files-in-the-repo-no-runtime-no-server.md)
- detailed_by [ADR-0011 — recursive scan; hub projects are wings](../decisions/0056-hubs-addressed-by-remote-located-by-derivation.md)
- detailed_by [ADR-0012 — wings optional; standalone hubs](../decisions/0056-hubs-addressed-by-remote-located-by-derivation.md)
- feeders_from [ADR-0005 — one canonical memory; others are feeders](../decisions/0051-the-ladder-runs-before-anything-is-remembered.md)
- recall_from [ADR-0006 — two-layer recall](../decisions/0058-recall-one-bounded-index.md)
- mines [ADR-0007 (now ADR-0050) — mine agentmemory's design](../decisions/0050-mage-turns-a-repeated-failure-into-enforcement.md)
- rides [ADR-0009 — no runtime; automation rides host hooks](../decisions/0054-files-in-the-repo-no-runtime-no-server.md)
