 ...
 *
 * THROWS if `note.frontmatter.type` is not "procedure" or "gotcha" (structural gate).
 */
export function renderProcedureSkill(
  ...
  if (!GRADUATABLE_TYPES.has(type)) {
    throw new Error(
      `mage graduate: only procedure/gotcha notes graduate to a skill (got type "${type || "(none)"}"). ` +
