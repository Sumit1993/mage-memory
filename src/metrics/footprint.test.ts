...
`;
  ...
  it("excludes generated artifacts stamped by OTHER generators (Dashboard.md)", async () => {
    const dir = await tmpDir("mage-footprint-");
    const docsRoot = join(dir, "mage");
    const notesRoot = join(docsRoot, "notes");
    await mkdir(notesRoot, { recursive: true });

    await writeFile(join(notesRoot, "note1.md"), "---\n---\nbody1");
    await writeFile(join(notesRoot, "note2.md"), "---\n---\nbody2");

    // `mage dashboard` stamps a DIFFERENT marker than `mage index`. An exact-match
    // sniff for the `mage index` marker counted this as a real note.
    await writeFile(
      join(docsRoot, "Dashboard.md"),
      "<!-- GENERATED by `mage dashboard` — do not edit by hand. -->\ndashboard body",
    );

    const footprint = await measureFootprint(docsRoot);
    expect(footprint.yield.notesTracked).toBe(2);
    expect(footprint.yield.notesNeverRead).toBe(2);
  });

  it("an AUTHORED file sharing a reserved basename under notes/ is still counted", async () => {
    const dir = await tmpDir("mage-footprint-");
    const docsRoot = join(dir, "mage");
    const notesRoot = join(docsRoot, "notes");
    await mkdir(notesRoot, { recursive: true });

    await writeFile(join(notesRoot, "INDEX.md"), "---\n---\nauthored");
    await writeFile(
    ...
    const footprint = await measureFootprint(docsRoot);
    expect(footprint.yield.notesTracked).toBe(1);
  });

  it("excludes generated index files from yield counts based on GEN_MARKER", async () => {
    const dir = await tmpDir("mage-footprint-");
    const docsRoot = join(dir, "mage");
    const notesRoot = join(docsRoot, "notes");
    await mkdir(notesRoot, { recursive: true });
    ...
    await writeFile(join(docsRoot, "INDEX.md"), `${GEN_MARKER}\nindex body`);
    await writeFile(join(docsRoot, "MEMORY.md"), `${GEN_MARKER}\nmemory body`);
    await writeFile(join(docsRoot, "_index.foo.md"), `${GEN_MARKER}\nwing body`);
