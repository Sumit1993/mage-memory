...
function renderMemory(
  ...
  if (content.endsWith("\n")) lines--;
  if (
    Buffer.byteLength(content, "utf8") <= byteThreshold &&
    lines <= lineThreshold
  )
    return { content, tier: 0 };
  ...
  if (lines <= lineThreshold) {
    ...
    content = renderFlat(entries, wings, reg, 1, msg1, acceptedDecisionsCount);
  ...
  const triggeringDimension =
    lines > lineThreshold ? `${capLines}-line` : `${capBytes} B`;
  const omitted =
    lines > lineThreshold
      ? "per-note lines omitted"
      : "keyword tails and per-note lines omitted";
