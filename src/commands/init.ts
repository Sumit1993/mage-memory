...
export interface InitOptions {
  ...
  mode?: InitMode;
  /** External mode: hub name (default `<repo>-mage`). */
  name?: string;
  ...
  project?: string;
  /** Skip the `-mage` suffix suggestion for external hub names. */
  noSuggest?: boolean;
...
async function resolveMode(opts: InitOptions): Promise<InitMode> {
  ...
  return (await select<InitMode>({
    ...
    choices: [
      ...
      {
        name: "External hub  (recommended for multi-repo / privacy-from-code / cross-cutting knowledge)",
        value: "external",
      },
    ],
    default: "in-repo",
...
async function resolveHubName(codeRepo: string, opts: InitOptions): Promise<string> {
  ...
  const defaultName = `${basename(codeRepo)}-mage`;
...
async function initExternal(args: ExternalArgs): Promise<string> {
  ...
  // 4. Create the hub-owned project's vault (notes/work/decisions/archive + INDEX).
  const projectDocsRoot = hubProjectDocsRoot(hubDir, project);
  await mkdir(projectDocsRoot, { recursive: true });
  await scaffoldVaultDirs(projectDocsRoot);
  logger.detail(`Created project vault: projects/${project}/mage/`);

  // 5. Git-ignore artifacts + scratch across the hub (ADR-0003).
  const added = await ensureGitignored(hubDir, ["**/artifacts/", ".learnings/", "**/.learnings/"]);
  if (added.length > 0) logger.detail(`Added .gitignore patterns: ${added.join(", ")}`);

  // 6. Portable navigation contract at the hub root.
  await writeAgentsMd(hubDir, { docsRel: ".", kind: "hub" });

  // 7. Write the code repo's metadata pointing at the hub
  const codeMeta: MageMetadata = {
    schema: METADATA_SCHEMA,
    mode: "external",
    project,
    hub_path: hubDir,
  ...
  logger.detail(`  git -C ${codeRepo} commit -m "feat: mage init (external, hub=${name})"`);
  logger.blank();
