...
export interface InitResult {
  ...
  /** What `connect` wired, when auto-connect ran (in-repo, Decision 5); absent if skipped or hub mode. */
...
export async function init(opts: InitOptions = {}): Promise<InitResult> {
  ...
  if (mode === "in-repo") {
    ...
    // connect failure (e.g. a malformed settings.local.json) must NOT fail init.
    let connectResult: ConnectResult | undefined;
    if (opts.connect !== false) {
      logger.info("Wiring capture hooks (pass --no-connect to skip)…");
      try {
        connectResult = await connect({ cwd, yes: opts.yes, gitHook: true });
      } catch (err) {
        logger.warn(
          `Knowledge base created, but auto-connect failed: ${(err as Error).message} — ` +
            "run `mage connect` to wire capture hooks.",
        );
      }
    }
    return { mode, codeRepo: cwd, project, hubDir: null, hubRepoUrl: null, connectResult };
  }
...
async function initHubFlow(cwd: string, opts: InitOptions): Promise<InitResult> {
  ...
  logger.blank();
  logger.info(
    "Hub created — it has no members yet. Run `mage link <hub>` from each code repo " +
      "(it auto-connects capture there); the hub dir itself is not a code repo, so it isn't connected.",
  );
