...
export async function status(opts: StatusOptions): Promise<StatusResult> {
  ...
  for (const repoInput of opts.codeRepos) {
    ...
    if (await looksLikeHub(repo)) {
      const retired = ["code", "repo", "path"].join("_");
      throw new Error(
        `${repo} is a mage hub; mage status checks code repos — pass each code repo path (hub expansion went with ${retired}, ADR-0047 §4)`,
      );
    }
...
async function statusOne(repo: string): Promise<RepoStatus> {
  ...
  if (!meta) {
    rs.metadata = { ok: false, detail: "missing mage/metadata.json" };
    return rs;
  }
  rs.metadata = {
    ok: true,
