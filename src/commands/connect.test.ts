...
describe("connect", () => {
  it("connect into a fresh dir creates settings.local.json with all 12 mage groups", async () => {
  ...
  it("does NOT commandeer when auto-memory is disabled (12 groups, no autoMemoryDirectory)", async () => {
  ...
  it("re-connect is idempotent", async () => {
    ...
    expect(r2.wired).toBe(12);
    expect(after2).toBe(after1);

    // still exactly 12 mage groups (no duplication)
...
describe("reach tier — connect grants out-of-repo KB access (ADR-0042)", () => {
  ...
  it("an absent hub does not abort connect — hooks still wire (regression)", async () => {
    ...
    // 12 base hooks: resolveDocsRoot returns null when hub is absent, so commandeer
...
describe("connect — legacy orphan reaping (#150)", () => {
  ...
  it("the live dark state — 36 id-less orphans BESIDE 12 tagged groups — collapses to exactly 12", async () => {
    const dir = await freshDir();
    // Measured shape of ~/.claude/settings.json on 2026-08-22: a pre-id mage's groups
    // never reaped, so 48 registrations fire where 12 should.
    ...
    expect(Object.values(seeded).flat()).toHaveLength(48);

    const r = await connect({ cwd: dir, yes: true, gitHook: false });
    expect(r.reaped).toBe(36);
