...
describe("mage index", () => {
  ...
  it("cleans stale generated wing index files and reports them in deleted", async () => {
    const dir = await vault();
    const staleWing = join(dir, "mage", "_index.oldwing.md");
    await writeFile(staleWing, "<!-- GENERATED by `mage index` — do not edit by hand. -->\n# old wing\n");
    const r = await index({ dir });
    expect(r.deleted).toEqual(["_index.oldwing.md"]);
    expect(existsSync(staleWing)).toBe(false);
  });
