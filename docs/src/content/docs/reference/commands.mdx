...
discover your knowledge base. Each generated wing skill includes a "Governing decisions" section listing all accepted, non-superseded decision records (ADRs) linked from that wing's notes. If the list would breach the recall/footprint ceilings, it is truncated keeping the most recent decisions (use `mage footprint`/`mage doctor` to diagnose breaches).
