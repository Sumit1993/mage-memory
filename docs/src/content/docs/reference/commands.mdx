...
**C · Plumbing** — hidden CLI verbs you never type by hand (`observe`, `distill`, `promote`, `stage`, `index`, `nudge`, `memory-hook`, `redact`, `ingest`). A skill or a host hook runs them for you; they appear in [the full inventory](#the-full-inventory) for transparency.
