...
**C · Plumbing** — hidden CLI verbs you never type by hand (`observe`, `distill`, `promote`, `stage`, `nudge`, `memory-hook`, `redact`, `ingest`, `flatten`, and friends). A skill or a host hook runs them for you; the complete, generated set is in [the full inventory](#the-full-inventory) — that table, not this list, is the authoritative roster.
...
- **Per-wing skills (`mage-wing-*`)** are *derived from your notes*, exactly like
  the index. `mage skills` is their writer; re-running it rebuilds them from the
  current wings, deterministically.
- **Graduated skills (`mage-skill-*`)** come out of the grooming loop, when a
  ...
  **single writer** that mints those, and the ceilings it enforces are the reason
  it is the only one. `mage skills` never creates, edits, or deletes a graduated
  skill. See [Promote and graduate](../loop/promote-graduate.md).
...
The hooks and the `/mage:learn` flow (the deliberate-capture skill that drafts a
note for you and writes it after you confirm) also run it for you
automatically, so you rarely need to. `mage skills` also carries a read-only metrics
