...
Run inside a code repo's `mage/`, `mage index` writes that one pair. Run at a
...
fan-out is one level deep only — calling `mage index` directly on a project root
writes that project's pair and nothing else. See [Set up a hub and external
