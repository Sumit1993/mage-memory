...
- **Clone-on-demand.** Per ADR-0043, when `mage connect` detects that a hub is missing at its derived path (`~/.mage/hubs/<host>/<owner>/<repo>`), it offers to clone it on demand.
