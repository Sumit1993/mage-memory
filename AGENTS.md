...
the **memory** genre only (`mage/notes/`). `mage/decisions/` (ADRs) is authored
deliberately, not a destination for captured knowledge. `mage/work/` is retired
...
ADR-0050 is the charter; the plan, its mechanics and its evidence are comments on that issue (#204).
